...
use vulkano::pipeline::{GraphicsPipeline, vertex::TwoBuffersDefinition};
...
impl GraphicsContext {
    ...
    pub fn draw(&mut self) {
        let buf = CpuAccessibleBuffer::from_iter(self.device.clone(), BufferUsage::all(), (0..1024*1024*4).map(|_| 0u8))
            .expect("failed to create buffer");

        let image = StorageImage::new(self.device.clone(), Dimensions::Dim2d { width: 1024, height: 1024 },
        Format::R8G8B8A8Unorm, Some(self.queue.family())).unwrap();

        let render_pass = Arc::new(vulkano::single_pass_renderpass!(self.device.clone(),
        attachments: {
            color: {
                load: Clear,
                store: Store,
        ...
        let vertex_buffer = self.geometry.vertices.iter().map(|vertex|{
            Vertex {
                position: [vertex.x, vertex.y],
            }
        });
        let vertex_buffer = CpuAccessibleBuffer::from_iter(self.device.clone(), BufferUsage::all(), vertex_buffer).unwrap();
        let index_buffer = CpuAccessibleBuffer::from_iter(self.device.clone(), BufferUsage::all(), self.geometry.indices.iter().cloned()).unwrap();
    ...
    pub fn new_circle(&mut self, pos: Point, rad: f32) {
        let options = FillOptions::tolerance(0.001);
        basic_shapes::fill_circle(pos, rad, &options, &mut simple_builder(&mut self.geometry));
    }
}
